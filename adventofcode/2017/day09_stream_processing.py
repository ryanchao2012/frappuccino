...
def cancel_charactor(raw: str) -> str:
    return re.sub(r'!.', '', raw)


def remove_garbage(raw: str) -> str:
    return re.sub(r'(<[^>]*>,?)|(,?<[^>]*>)', '', raw)  # https://regex101.com/
...
def scoring(grouping: str) -> int:
    depth: int = 0
    groups: List[int] = []
    pairs: List[Group] = []

    for idx, char in enumerate(grouping):
        if char == '}':
            groups.append(Group(depth, pairs.pop(), idx))
            depth -= 1
        elif char == '{':
            depth += 1
            pairs.append(idx)

    return sum(g.depth for g in groups)
...
class Part01Test(unittest.TestCase):
    ...
    def test_cancel(self):
        ...
        for line, exp in zip(inputs.strip().split('\n'), expects):
            ...
            self.assertEqual(cancel_charactor(line.strip()), exp)
    ...
    def test_remove_garbage(self):
        ...
        for line, exp in zip(inputs.strip().split('\n'), expects):
            self.assertEqual(remove_garbage(line.strip()), exp)
    ...
    def test_scoring(self):
        ...
        expects = [1, 6, 5, 16, 9, 3]

        for line, exp in zip(inputs.strip().split('\n'), expects):
            self.assertEqual(scoring(line.strip()), exp)
...
def part01(path: str):
    ...
    cleaned = remove_garbage(cancel_charactor(line))

    print('- Part01 Answer:', scoring(cleaned))
...
def accumulate_garbage(raw: str) -> int:
    # https://regex101.com/
    return sum(len(m.group()) - 2 for m in re.finditer(r'<[^>]*>', raw))


class Part02Test(unittest.TestCase):

    def test_accum_garbage(self):
        ...
        for line, exp in zip(inputs.strip().split('\n'), expects):
            ...
            self.assertEqual(accumulate_garbage(line.strip()), exp)
...
def part02(path: str):
    ...
    cleaned = cancel_charactor(line)

    print('- Part02 Answer:', accumulate_garbage(cleaned))


if __name__ == '__main__':
    part01('day09_input.txt')  # 11089
    part02('day09_input.txt')  # 5288
