...
class DuelGenerator:
    ...
    __slots__ = ('_value', '_multiplier', '_modulor', '_criteria')

    def __init__(self,
                 prior: int,
                 multiplier: int = 16807,
                 modulor: int = 2147483647,
                 criteria: int = 0):
        self._value = prior
        self._multiplier = multiplier
        self._modulor = modulor
        self._criteria = criteria

    def evolve(self):
        value = (self._value * self._multiplier) % self._modulor

        if self._criteria > 0:
            while value % self._criteria != 0:
                value = (value * self._multiplier) % self._modulor
        self._value = value

        return value


def duel_judge(A: DuelGenerator, B: DuelGenerator, jround: int = 40000000):

    judge = 2**16
    agree = 0
    for _ in range(jround):
        if A.evolve() % judge == B.evolve() % judge:
            agree += 1
...
class Part01Test(unittest.TestCase):
    def test_gen_A(self):
        gen_a = DuelGenerator(65, multiplier=16807)
        actual = [gen_a.evolve() for _ in range(5)]
    ...
    def test_gen_B(self):
        gen_b = DuelGenerator(8921, multiplier=48271)
        actual = [gen_b.evolve() for _ in range(5)]
    ...
    def test_judge(self):
        ...
        self.assertEqual(duel_judge(gen_a, gen_b), expect)


def part01(prior_a: int, prior_b: int):
    ...
    gen_a = DuelGenerator(prior_a, multiplier=16807)
    gen_b = DuelGenerator(prior_b, multiplier=48271)

    print('- Part01 Answer:', duel_judge(gen_a, gen_b))
...
The judge still waits for each generator to provide it
with a value before comparing them (using the same comparison method as before).
...
class Part02Test(unittest.TestCase):
    def test_gen_A(self):
        gen_a = DuelGenerator(65, multiplier=16807, criteria=4)
        actual = [gen_a.evolve() for _ in range(5)]
    ...
    def test_gen_B(self):
        gen_b = DuelGenerator(8921, multiplier=48271, criteria=8)
        actual = [gen_b.evolve() for _ in range(5)]
    ...
    def test_judge(self):
        ...
        self.assertEqual(duel_judge(gen_a, gen_b, jround=5000000), expect)


def part02(prior_a: int, prior_b: int):
    ...
    gen_a = DuelGenerator(prior_a, multiplier=16807, criteria=4)
    gen_b = DuelGenerator(prior_b, multiplier=48271, criteria=8)

    print('- Part02 Answer:', duel_judge(gen_a, gen_b, jround=5000000))


if __name__ == '__main__':

    part01(prior_a=634, prior_b=301)  # 573
    part02(prior_a=634, prior_b=301)  # 294
