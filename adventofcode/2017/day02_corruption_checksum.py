...
def format_sheet(sheet: str) -> List[List[int]]:
    return [[int(v) for v in row.strip().split()] for row in sheet.strip().split('\n')]


def checksum(sheet: List[List[int]]) -> int:
...
def part01():
    ...
    sheet = format_sheet(inputs)
    print('- Part01 Answer:', checksum(sheet))  # 42378
...
where one evenly divides the other - that is, where the result of the division operation is a whole number.
...
def find_division(arr: List[int]) -> int:
    sorted_arr = sorted(arr)
    for i, value in enumerate(sorted_arr):
        if value <= 0:
            continue
        for bigger_value in sorted_arr[i+1:]:
            if bigger_value % value == 0:
                return bigger_value // value

    raise ValueError('No divisible pair.')


def divisible_checksum(sheet: List[List[int]]) -> int:
    return sum([find_division(row) for row in sheet])
...
class Part02Test(unittest.TestCase):
    ...
    def test_divisible_checksum(self):
        sheet = [
            [5, 9, 2, 8],
            [9, 4, 7, 3],
            [3, 8, 6, 5]
        ]
        actual = divisible_checksum(sheet)
        expected = 9
        self.assertEqual(actual, expected)
...
def part02():
    ...
    sheet = format_sheet(inputs)
    print('- Part02 Answer:', divisible_checksum(sheet))  # 246
