...
class Part01Test(TC):
    def test_sum(self):
        cases = [(int(e) for e in '+1\n+1\n+1'.split()),
                 (int(e) for e in '+1\n+1\n-2'.split()),
                 (int(e) for e in '-1\n-2\n-3'.split())]
        expects = [3, 0, -6]

        for case, expect in zip(cases, expects):
            self.assertEqual(sum(case), expect)
...
class Part02Test(TC):
    def test_find_repeated(self):
        cases = [(int(e) for e in '+1, -1'.split(', ')),
                 (int(e) for e in '+3, +3, +4, -2, -4'.split(', ')),
                 (int(e) for e in '-6, +3, +8, +5, -6'.split(', ')),
                 (int(e) for e in '+7, +7, -2, -7, -4'.split(', '))]
        expects = [0, 10, 5, 14]

        for case, expect in zip(cases, expects):
            self.assertEqual(find_repeated(case), expect)
...
if __name__ == "__main__":
    part01('day01_input.txt')  # 556
    part02('day01_input.txt')  # 448
